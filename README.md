# Testing Express API Routes the Easy Way
Unit testing is one of the best practices in software development. In the development of web APIs, there not question how important unit testing is. In web APIs, it it important to test and be sure that all your routes are returning the responses you expect from them. 

Testing routes in a web API can be a nightmare if you don't structure your components for easy testing. One area that causes the most pain for beginners in testing API routes is routes that involve database access. This is inevitable because most of the times you are going to build CRUD applications as a beginner. In this article, we will learn how to test API routes with the least amount of difficulty possible. 

## What will this article cover?
The main focus of this article is only to show you how to test routes to ensure that you they return the exact responses that you expect from them. Therefore, other related testing activities such as database testing will not be covered in this article.
...
- Unit testing with JavaScript unit testing libraries like [jest](https://jestjs.io/).
...
With the introduction and pleasantries out of the way, here is the order in which we are going to learn how to test API routes the easy way:
...
5. Write your test involving database operation.
6. Decouple your data-access layer from the  web layer.
7. Introduce dependencies on the controller and  the router.
8. Mock your data-access layer.


## 1. Configure a test environment.
In the popular spirit of "Do not repeat yourself", we will use a prexisting test environment. We will [clone a pre-configure test environment](https://github.com/GHOST-Aram/ts-jest-env) from a Github. This repository contains configurations for Jest and Typescript. If you are interested, [visit this article](https://dev.to/ghostaram/configuring-jest-for-typescript-unit-tests-4iag) to learn how to configure a test environment for Jest and Typescript.
...
```
There we go, that's all we need for now. Notice that we are not connecting to a database or listening on a port. We do not need database connectivity or a running instance of our applicatio to be able to test it.

## 3. Modularize your application for easy testing.
To enable us to efficiently test our application routes, we need to decouple it into its individual components. In this section, we will decouple our API into routes, controller and the main file(`app.ts`).
...
Import the router in `app.ts` and replace the route handler as below.
...
We have successfully modularized out application for easy testing. Get ready to write your first route test in the next step.

## 4. Write your first route test.
In the previous sections, we have created a simple Express API and modularised it for easy testing. Let us write a test for the index (`/`) route of our application. 
...
### b). Install a route testing library
...
Notice that route we are testing does not exist in our application just yet. We are doing this intentionally for this test to fail. It is a good practice to have a failing test before we can write any code to make it pass. 
...
Success! You have writen and ran your fast test. Let us try testing a `POST` request that saves data to the database in the next step.


### 5. Write a test involving database operation.
We have seen how to test a simple `GET` route that only responds with a message. Now let us create a test for a `POST` route    `/item` that will do the following 
...
The test for the route that satifsied the above requirements would look like the following.
...
```
Right now all the test we have just added are failing as in the screen shot below.
...
Before we try to make the test pass, we need to create a model that we are going to use to implement the database operation. Here is the code snippet for the model we will use.

To create a model, we use the `mongoose` module. Install mongoose using the command below.
...
Remember that we reserve the implementation of the function until it is the last hting we need to do for the test to pas. 
...
This is because for one reason, we are not connecting to the database. Connecting to the database would not be of greate use either. Let us see how to reslove this in the next steps.

## 6. Decouple data-access from the web layer.
...
With that, we are two steps close to resolving the problem. Lets introduce a dependency on the `router` in the next step.
...
We will first encapsulate the data access layer.
...
### d). Encapsulate the controller web layer
...
In this step, we will create a new class with a function that imitate the behavior of the `APIDAL` class. We will create the file and the class with the same names in a different directory.
...
In the above code snippet, we have replace the original function implementation with a mock.

Now if you run your tests, they should all pass as in the screenshot below.

![All tests passing](https://dev-to-uploads.s3.amazonaws.com/uploads/articles/fulpyivo0b8nui3tmj1h.png)

## Conclusion
Finally, we have successfully explored and practices how to test Express API routes the easy way. In this article we have learned how to do the following:
...
All the code we have written on this article is available in [this GitHub repository](https://github.com/GHOST-Aram/api-testing-the-easy-way)
